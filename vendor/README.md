...
├── windows/                # Windows 二进制（adb.exe, scrcpy.exe, *.dll）
├── macos/                  # macOS 二进制（adb, scrcpy）
└── linux/                  # Linux 二进制（adb, scrcpy）
```
...
AudioRouter 不放这里，仍在 `AudioRouter/` 顶层 CMake 项目中。
代码会按平台在 `AudioRouter/` 下查找 `AudioRouter.exe` / `AudioRouter`。
